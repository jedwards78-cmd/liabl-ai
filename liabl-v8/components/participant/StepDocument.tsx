...
export default function StepDocument({ clauses, answers, onNext, onBack }: Props) {
  ...
  return (
    <div className="card">
      ...
      <h2 className="font-serif text-2xl mb-1">Your personalized waiver</h2>
      ...
      <div className="flex gap-2 flex-wrap mt-3 mb-4">
        {[answers.fullName, actLabel, date].map(tag => (
          <span key={tag} className="bg-surface border border-black/10 text-xs px-2.5 py-1 rounded-full text-gray-500">
            {tag}
          </span>
        ))}
      </div>

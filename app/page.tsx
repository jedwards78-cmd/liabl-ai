...
const ACTS: { key:HeroActivity; emoji:string; label:string }[] = [
  { key:'kayak', emoji:'🚣', label:'Kayaking'  },
  { key:'hike',  emoji:'🥾', label:'Hiking'    },
  { key:'atv',   emoji:'🏎️', label:'ATV Tour'  },
  { key:'climb', emoji:'🧗', label:'Climbing'  },
]
...
const TILES = [
  { href:'/participant', label:'Waivers That Build Themselves',          sub:'See the document adapt in real time',       bg:'#4B2ACF', emoji:'✍️' },
  { href:'/portal',      label:'A Participant Profile That Remembers',   sub:'Cross-operator identity & history',         bg:'#0891B2', emoji:'✦'  },
  { href:'/operator',    label:'Know Before They Show Up',               sub:'AI risk intelligence at check-in',          bg:'#059669', emoji:'⚡' },
  { href:'/groups',      label:'Groups That Sign Before They Arrive',    sub:'Pre-arrival signing & manifest sync',       bg:'#7C3AED', emoji:'📋' },
  { href:'/integrations','label':'Connected to Everything You Already Use',sub:'FareHarbor, Rezdy, SSO & insurance',    bg:'#D97706', emoji:'🔗' },
  { href:'/security',    label:'A Legal Record That Holds Up in Court',  sub:'Audit trail, integrity & compliance',       bg:'#DC2626', emoji:'🔒' },
  { href:'/pricing',     label:'Built for Every Stage of Growth',        sub:'Feature-based plans from $49/mo',           bg:'#0D9488', emoji:'💳' },
  { href:'/why',         label:'The Market Window — and Why It\'s Now',  sub:'Market case & investor overview',           bg:'#BE185D', emoji:'📡' },
]
    ...
    <main className="min-h-screen bg-surface">
      ...
      <nav className="bg-white border-b border-black/10 px-6 py-3 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/participant" className="text-sm px-4 py-2 bg-brand text-white rounded-xl font-medium hover:opacity-90 transition-all">
            Try it live →
          </Link>
        </div>
      </nav>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <h1 className="font-serif text-4xl sm:text-5xl text-ink leading-tight mb-5" style={{ letterSpacing:'-0.025em' }}>
                ...
                <span className="text-brand">It should be thinking.</span>
              </h1>
              <p className="text-gray-500 text-lg leading-relaxed mb-4">
                LIABL builds the document from the participant&apos;s answers in real time —
            ...
            {/* Right — live adaptive demo (item B: hero placement) */}
            <div className="bg-surface rounded-2xl border border-black/10 overflow-hidden">
              <div className="bg-ink px-5 py-3 flex items-center justify-between">
                <span className="text-white text-xs font-semibold">⚡ Live — document building in real time</span>
                {adaptiveCount > 0 && (
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="font-serif text-3xl sm:text-4xl mb-3" style={{ letterSpacing:'-0.02em' }}>
              Traditional waivers are static. Ours aren&apos;t.
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                {[
                  { icon:'📄', text:'Operator writes a template once. Every participant gets the same document.' },
                  { icon:'📁', text:'Completed form stored in a folder. No intelligence. No identity. No network.' },
                  { icon:'🔍', text:'When something goes wrong, someone searches manually through files.' },
                  { icon:'🔄', text:'Every new participant starts from scratch, regardless of history with your business.' },
                  { icon:'📊', text:'No risk signals until after the incident. No data that improves over time.' },
                ].map(({ icon, text }) => (
            ...
            <div className="bg-brand/15 border border-brand/30 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                {[
                  { icon:'⚡', text:'Document builds itself from participant answers. A cardiac condition changes the waiver — not just adds a note.' },
                  { icon:'🧠', text:'Every signature is a data event. Every answer is a risk signal. The platform gets smarter with every signing.' },
                  { icon:'🔒', text:'SHA-256 sealed at signing time. Full chain of custody produced in seconds. Forensically complete.' },
                  { icon:'✦',  text:'Returning participants recognized across every LIABL operator. 15-second re-sign. Identity that compounds.' },
                  { icon:'📡', text:'Real-time risk intelligence at check-in. Shared with your insurer automatically. Defensible before the session starts.' },
                ].map(({ icon, text }) => (
                  <div key={text} className="flex items-start gap-3 text-sm text-white">
            ...
            {[
              {
                ...
                sub:'Adaptive waivers, booking platform sync, and a returning participant experience that takes 15 seconds. LIABL gives you that time back on day one.',
              ...
              },
              {
                ...
                icon:'🔗',
                headline:'Your operators keep asking for waiver features. We built them.',
      ...
      <section className="px-6 py-16 bg-white border-b border-black/8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            ].map(({ icon, title, lines, result, net, color, bg }) => (
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 6 — EXPLORE THE PLATFORM (transformed tiles)
      ══════════════════════════════════════════════════════ */}
      <section className="px-6 py-16 bg-surface border-b border-black/8">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {TILES.map(({ href, label, sub, bg, emoji }) => (
              <Link key={href} href={href}
                className="bg-white rounded-2xl border border-black/10 hover:border-black/20 hover:shadow-md transition-all p-5 group">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl mb-3 group-hover:scale-105 transition-transform"
                  style={{ background: bg }}>
                  <span>{emoji}</span>
                </div>

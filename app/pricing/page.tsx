...
const TIERS = [
  {
    id:'core', name:'Core', tagline:'Smart waivers. Flat fee. No surprises.',
    monthly:49, annual:39, highlight:false,
    features:['Adaptive document engine','Unlimited participants & signatures','Guardian flow for minors','Operator roster & check-in','Email confirmations','90-day document retention','1 operator user seat'],
    missing:['LIABL Pass','Group reservations','Booking integrations','Analytics & export','API access'],
  },
  {
    id:'connected', name:'Connected', tagline:'Your booking platform and LIABL — always in sync.',
    monthly:149, annual:119, highlight:true,
    features:['Everything in Core','LIABL Pass (cross-operator)','Group reservations (all 4 tabs)','FareHarbor, Rezdy, Xola, Bókun','Bidirectional webhooks','Analytics + CSV export','Staff notes & addendum workflow','3-year document retention','5 operator user seats','API read access'],
    missing:['Role-based access control','Legal hold & lifecycle mgmt','SSO (Azure / Okta)'],
  },
  {
    id:'intelligence', name:'Intelligence', tagline:'The compliance layer your legal team has been asking for.',
    monthly:349, annual:279, highlight:false,
    features:['Everything in Connected','Full analytics suite','Document lifecycle management','Legal hold & retention policies','Role-based access control','Metadata tagging','Audit trail & hash verification','GDPR/CCPA deletion workflow','7-year document retention','Unlimited user seats','Full API access (read/write)','Priority support'],
    missing:[],
  },
]
    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="max-w-4xl mx-auto px-4 py-14">
          ...
          {TIERS.map(t => {
            ...
            return (
              <div key={t.id} className={`rounded-2xl border p-6 flex flex-col ${
                t.highlight ? 'border-brand bg-brand text-white shadow-xl shadow-brand/20 scale-[1.02]' : 'border-black/10 bg-white'
              }`}>
                ...
                <button className={`w-full py-3 rounded-xl font-semibold text-sm transition-all ${t.highlight?'bg-white text-brand hover:bg-white/90':'bg-brand text-white hover:opacity-90'}`}>
                  Start free trial
                </button>
              </div>
            )
        ...
        <div className="text-center mb-8">
          <button onClick={() => setShowCompare(!showCompare)} className="text-sm text-brand underline hover:opacity-70 transition-opacity">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-ink rounded-2xl p-6 text-white">
            ...
            <h3 className="font-serif text-xl mb-2" style={{letterSpacing:'-0.01em'}}>Multi-location &amp; compliance-first operators</h3>
            <p className="text-gray-400 text-sm leading-relaxed mb-4">SSO (Azure/Okta), SCIM provisioning, custom data residency, SLA, dedicated support, multi-location unified reporting, and volume pricing.</p>
            <div className="space-y-1.5 mb-5">
              {['SSO + SCIM provisioning','Custom data residency','Dedicated support & SLA','Multi-location reporting','Volume pricing'].map(f => (
          ...
          <div className="bg-brand/5 border border-brand/20 rounded-2xl p-6">
            ...
            <h3 className="font-serif text-xl text-ink mb-2" style={{letterSpacing:'-0.01em'}}>Booking platforms &amp; franchise operators</h3>
            <p className="text-gray-500 text-sm leading-relaxed mb-4">Embed LIABL&apos;s document layer under your own brand. Full UI white-labeling, custom domain, platform-level API, and revenue share model.</p>
            <div className="space-y-1.5 mb-5">
              {['Full UI white-labeling','Custom domain & branding','Platform-level API access','Revenue share model','Dedicated integration engineering'].map(f => (

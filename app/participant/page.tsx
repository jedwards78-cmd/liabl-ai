...
// Steps: 0=entry 1=identity 2=activity 3=health 4=guardian(if minor) 5=document 6=sign 7=confirm
const STEP_LABELS_ADULT = ['Identity', 'Activity', 'Health', 'Review', 'Sign']
const STEP_LABELS_MINOR = ['Identity', 'Activity', 'Health', 'Guardian', 'Review', 'Sign']
...
export default function ParticipantPage() {
  ...
  const stepLabels = isMinor ? STEP_LABELS_MINOR : STEP_LABELS_ADULT

  // Maps logical step to progress bar index (step 0 = entry, no bar)
  ...
  function next(update?: Partial<ParticipantAnswers>) {
    ...
    // If minor and we just did health (step 3), go to guardian (step 4)
    // Otherwise skip guardian for adults
    const nextStep = step === 3 && !merged.isMinor ? 5 : step + 1
    setStep(nextStep)
  }

  function prev() {
    // If on document step (5) and adult, go back to health (3)
    const prevStep = step === 5 && !isMinor ? 3 : step - 1
    ...
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center px-4 py-8">
        <div className="w-full max-w-lg">
          {step > 0 && step < 7 && (
            <ProgressBar steps={stepLabels} current={Math.min(progressIndex, stepLabels.length - 1)} />
          )}
          ...
          <div className="animate-fade-up" key={step}>
            ...
            {step === 4 && isMinor && (
              <StepGuardian
                minorName={answers.fullName ?? 'Minor'}

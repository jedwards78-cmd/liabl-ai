...
const FORCES = [
  { num:'01', icon:'🏔', title:'Activity industry growth', sub:'The market is expanding faster than the tooling', color:'text-brand', bg:'bg-brand/5 border-brand/20',
  ...
  { num:'02', icon:'⚖️', title:'Rising liability exposure', sub:'Operators know a bad waiver is worse than no waiver', color:'text-amber-600', bg:'bg-amber-50 border-amber-200',
  ...
  { num:'03', icon:'🔌', title:'Booking platform API maturity', sub:'The integration layer just became capable', color:'text-emerald-600', bg:'bg-emerald-50 border-emerald-200',
  ...
  return (
    <div className="min-h-screen bg-surface">
      <nav className="bg-white border-b border-black/10 px-5 py-3 flex items-center justify-between">
        <Logo size="md"/>
        <span className="text-xs bg-surface border border-black/10 px-3 py-1.5 rounded-full text-gray-500">Why LIABL · Why now</span>
      </nav>
      ...
      <div className="bg-ink text-white px-6 py-14 text-center">
        <p className="text-xs font-semibold tracking-widest uppercase mb-4" style={{color:'#A78BFA'}}>{tab==='why'?'Market thesis':tab==='investor'?'Investor overview':'Sales Demo'}</p>
        ...
        <p className="text-gray-400 text-base max-w-xl mx-auto leading-relaxed">
          {tab==='why'?"Activity industry growth, rising liability exposure, and booking platform API maturity have created a moment that didn't exist three years ago.":tab==='investor'?"An interactive walkthrough of LIABL's market position, adaptive document intelligence, and the network effect thesis.":"Choose a buyer persona to launch a scripted, step-by-step demo tailored to their specific pain points and decision criteria."}
        </p>
      </div>
        ...
        {tab==='investor'&&(
          <div className="space-y-14">
            <section>
              ...
              <div className="bg-brand/5 border border-brand/20 rounded-xl p-5 flex items-start gap-3"><div className="w-8 h-8 rounded-lg bg-brand flex items-center justify-center text-white text-sm font-bold shrink-0">L</div><div><div className="font-semibold text-ink mb-1">LIABL fills the gap</div><div className="text-sm text-gray-500 leading-relaxed">New entrants cannot replicate LIABL's position — the participant identity graph requires years of signed documents to become defensible. The network effect is the moat.</div></div></div>
            </section>
            <section>

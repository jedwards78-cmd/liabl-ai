...
export function calculateRiskScore(factors: RiskFactors): { score: number; level: 'low' | 'moderate' | 'elevated' | 'high'; factors: string[] } {
  ...
  // Health
  if (factors.healthStatus === 'cardiac') { score += 30; contributing.push('Cardiovascular condition disclosed') }
  if (factors.healthStatus === 'injury')  { score += 20; contributing.push('Recent injury disclosed') }
  ...
  return (
    <div className={`rounded-xl border p-4 ${cfg.bg} ${cfg.border}`}>
      <div className="flex items-center justify-between mb-3">
        <div className={`font-semibold text-sm ${cfg.color}`}>⚡ AI Risk Assessment</div>

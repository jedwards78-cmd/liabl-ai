...
const SCIM_EVENTS = [
  ...
  { time:'Yesterday', event:'group.synced',     user:'All managers',  detail:'Role mapping updated after Azure group change' },
]
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 leading-relaxed max-w-2xl">
          Enterprise operators can authenticate their team through any major identity provider. LIABL supports SAML 2.0 and OIDC — no passwords to manage, no separate LIABL accounts for your staff.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          { icon:'🔑', title:'SP-initiated SSO', desc:'Staff click "Sign in with [Provider]" on LIABL. Redirected to your identity provider, authenticated, returned to LIABL automatically.' },
          { icon:'🏢', title:'IdP-initiated SSO', desc:'Staff click the LIABL tile in your Okta or Azure app portal. Authenticated in one click — no LIABL login page at all.' },
          { icon:'⚡', title:'Just-in-time provisioning', desc:'First time a user signs in via SSO, a LIABL account is created automatically. Role assigned from your IdP group mapping.' },
        ].map(p => (
              ...
              <div onClick={() => setExpanded(expanded === p.id ? null : p.id)}
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">
                    {p.connected ? `${p.since} · ${p.users} team members synced` : 'Not connected'} · {p.protocol}
                  </div>
                </div>
                  ...
                  {p.connected ? (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                          { label:'Entity ID / Issuer',     value:'https://auth.liabl.com/saml/azure' },
                          { label:'ACS URL',                value:'https://auth.liabl.com/saml/callback' },
                          { label:'Metadata URL',           value:'https://auth.liabl.com/saml/metadata.xml' },
                          { label:'Certificate fingerprint',value:'SHA-256: 4A:9F:2C...' },
                        ].map(({ label, value }) => (
                      ...
                      {/* Role mapping */}
                      <div>
                        <div className="text-xs font-semibold text-gray-500 mb-2">Group → role mapping</div>
                        <div className="space-y-1.5">
                          {[
                            { group:'LIABL-Owners',   role:'Owner' },
                            { group:'LIABL-Managers', role:'Manager' },
                            { group:'LIABL-Staff',    role:'Staff' },
                          ].map(({ group, role }) => (
                            <div key={group} className="flex items-center gap-2 text-xs bg-white rounded-lg border border-black/8 px-3 py-2">
                              <span className="font-mono text-gray-600 flex-1">{group}</span>
                              <span className="text-gray-400">→</span>
                              <span className="bg-brand/10 text-brand px-2 py-0.5 rounded-full font-medium">{role}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <button className="text-sm px-4 py-2 rounded-xl border border-black/20 text-gray-600 hover:bg-white transition-colors">Edit config</button>
                        <button className="text-sm px-4 py-2 rounded-xl border border-black/20 text-gray-600 hover:bg-white transition-colors">Test SSO</button>
                    ...
                    <div className="space-y-3">
                      <p className="text-xs text-gray-500 leading-relaxed">
                        To connect {p.name}, you&apos;ll need to create a LIABL app in your {p.name} admin portal and paste the metadata URL or XML below.
                      </p>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">Metadata URL or XML</label>
                        <div className="flex gap-2">
                      ...
                      <div className="text-xs text-gray-400">
                        Need help? See the{' '}
                        <span className="text-brand underline cursor-pointer">{p.name} setup guide →</span>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mb-4 leading-relaxed max-w-2xl">
          SCIM (System for Cross-domain Identity Management) keeps LIABL in sync with your identity provider automatically. When someone joins your org, their LIABL account is created. When they leave, access is revoked within minutes — no manual offboarding.
        </p>
        ...
        <div className="bg-white rounded-2xl border border-black/10 p-5 mb-4">
          <div className="space-y-4">
            ...
            <div className="space-y-3 pt-2 border-t border-black/8">
              {[
                { label:'Just-in-time (JIT) provisioning',    sub:'Create LIABL accounts automatically on first SSO login', val:jit,      set:setJit },
                { label:'SCIM automatic deprovisioning',      sub:'Revoke access when user is removed from IdP',            val:scim,     set:setScim },
                { label:'Enforce SSO (disable password login)',sub:'Staff must use SSO — no email/password fallback',        val:forceSSO, set:setForceSSO },
                ...
                <div key={label} className="flex items-center justify-between gap-4">
                  ...
                  <button onClick={() => set(!val)}
                    className={`w-11 h-6 rounded-full transition-all shrink-0 relative ${val ? 'bg-brand' : 'bg-gray-200'}`}>
          ...
          {SCIM_EVENTS.map((e, i) => (
            <div key={i} className="flex items-start gap-3 px-5 py-3 border-b border-black/5 last:border-0 text-xs">
              ...
              <span className={`px-2 py-0.5 rounded-full font-medium shrink-0 ${
                e.event === 'user.provisioned' ? 'bg-emerald-50 text-emerald-700' :
                e.event === 'user.deprovisioned' ? 'bg-red-50 text-red-600' : 'bg-brand/10 text-brand'
              }`}>{e.event}</span>
              <div>
                <div className="font-medium text-ink">{e.user}</div>

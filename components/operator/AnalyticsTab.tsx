...
function FunnelChart({ steps }: { steps:{label:string;value:number;pct:number}[] }) {
  return (
    <div className="space-y-2">
      {steps.map((s,i)=>(
        <div key={i}>
          <div className="flex justify-between text-xs mb-1"><span className="text-gray-600">{s.label}</span><span className="font-medium text-ink">{s.value} <span className="text-gray-400">({s.pct}%)</span></span></div>
          <div className="h-5 bg-surface rounded-lg overflow-hidden"><div className="h-full rounded-lg" style={{width:`${s.pct}%`,background:`rgba(75,42,207,${0.3+(s.pct/100)*0.7})`}}/></div>
        </div>
      ))}
    </div>
  )
...
const FUNNEL=[{label:'Waiver link opened',value:312,pct:100},{label:'Identity completed',value:289,pct:93},{label:'Activity selected',value:281,pct:90},{label:'Health answered',value:274,pct:88},{label:'Document reviewed',value:251,pct:81},{label:'Signature submitted',value:238,pct:76}]
const EXC=[{label:'May 1',value:1},{label:'May 5',value:0},{label:'May 10',value:2},{label:'May 15',value:1},{label:'May 20',value:3},{label:'May 25',value:1},{label:'May 30',value:2},{label:'Jun 3',value:2}]
    ...
    <div>
      <div className="flex items-start justify-between flex-wrap gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={doExport} className={`text-sm px-4 py-2 rounded-xl border transition-all ${exported?'bg-emerald-50 text-emerald-700 border-emerald-200':'bg-white border-black/20 text-gray-600 hover:bg-surface'}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[{label:'Total waivers',value:'238',delta:'+14%'},{label:'Completion rate',value:'76%',delta:'+3%'},{label:'Avg sign time',value:'1m 47s',delta:'-22s'},{label:'Exception rate',value:'4.2%',delta:'-0.8%'}].map(({label,value,delta})=>(
          <div key={label} className="bg-white rounded-xl border border-black/10 p-4">
            <div className="text-2xl font-semibold text-ink mb-1" style={{letterSpacing:'-0.01em'}}>{value}</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div className="bg-white rounded-2xl border border-black/10 p-5"><div className="text-sm font-semibold text-ink mb-1">Waivers signed over time</div><div className="text-xs text-gray-400 mb-3">Daily — last 30 days</div><LineChart data={TREND}/></div>
        ...
        <div className="bg-white rounded-2xl border border-black/10 p-5"><div className="text-sm font-semibold text-ink mb-1">Exception rate</div><div className="text-xs text-gray-400 mb-3">Last 30 days</div><LineChart data={EXC} color="#D97706"/></div>
        <div className="bg-white rounded-2xl border border-black/10 p-5 sm:col-span-2"><div className="text-sm font-semibold text-ink mb-1">Signing completion funnel</div><div className="text-xs text-gray-400 mb-3">Drop-off at each step</div><FunnelChart steps={FUNNEL}/></div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[{label:'Waiver data CSV',icon:'📄'},{label:'Participant list',icon:'👥'},{label:'Analytics report',icon:'📊'},{label:'Exception log',icon:'⚠'}].map(({label,icon})=>(
            <button key={label} onClick={doExport} className="text-left p-3 bg-white rounded-xl border border-black/10 hover:border-brand/30 hover:bg-brand/5 transition-all">
              <div className="text-xl mb-1">{icon}</div><div className="text-xs font-medium text-ink">{label}</div>
            </button>
          ))}
        </div>
      </div>

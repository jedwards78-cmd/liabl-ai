...
interface WaiverParticipant {
  name:string; email:string; dob:string; activity:string; signedAt:string
  ip:string; device:string; docId:string; hash:string; isMinor:boolean
  isReturning:boolean; visitCount:number; healthFlag:string|null
...
export default function WaiverDetail({ participant, onBack }: Props) {
  const [notes,         setNotes]         = useState<StaffNote[]>([{id:'n1',author:'J. Torres',time:'9:06 AM',content:'Participant arrived early. Reviewed safety briefing in full. No concerns.'}])
  ...
  function submitNote(){if(!newNote.trim())return;setNotes(n=>[...n,{id:`n${Date.now()}`,author:'You',time:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),content:newNote.trim()}]);setNewNote('')}
    ...
    <div>
      ...
      <div className="flex gap-1 bg-surface rounded-xl p-1 mb-6 w-fit">
        {(['document','identity','audit'] as const).map(s=>(
          <button key={s} onClick={()=>setSection(s)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all capitalize ${section===s?'bg-white text-ink shadow-sm':'text-gray-500 hover:text-ink'}`}>{s}</button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-black/10 p-5">
                <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
                  <div>
                    ...
                    <h2 className="font-serif text-xl" style={{letterSpacing:'-0.01em'}}>{participant.activity} — Liability Waiver</h2>
                    <div className="font-mono text-xs text-gray-400 mt-1">{participant.docId}</div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={()=>setEmailSent(true)} className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${emailSent?'bg-emerald-50 text-emerald-700 border-emerald-200':'border-black/20 text-gray-600 hover:bg-surface'}`}>{emailSent?'✓ Sent':'✉ Email copy'}</button>
                    <button className="text-xs px-3 py-1.5 rounded-lg border border-black/20 text-gray-600 hover:bg-surface">↓ Download PDF</button>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-black/8">
                  ...
                  <div className="bg-surface rounded-xl p-4 flex items-center gap-3">
                    <div className="flex-1"><div className="font-serif text-2xl text-brand italic mb-1">{participant.name}</div><div className="font-mono text-xs text-gray-400">{participant.signedAt} · {participant.ip} · ESIGN Act</div></div>
              ...
              {addendumSent&&(
                <div className={`bg-white rounded-2xl border overflow-hidden ${addendumSigned?'border-emerald-200':'border-amber-200'}`}>
                  <div className={`px-5 py-3 border-b flex items-center justify-between ${addendumSigned?'bg-emerald-50 border-emerald-200':'bg-amber-50 border-amber-200'}`}>
                    ...
                    {!addendumSigned&&<button onClick={()=>setAddendumSigned(true)} className="text-xs px-3 py-1 rounded-lg bg-amber-500 text-white">Simulate participant sign</button>}
                  </div>
          ...
          {section==='identity'&&(
            <div className="bg-white rounded-2xl border border-black/10 p-5 space-y-4">
              <h2 className="font-serif text-xl mb-2" style={{letterSpacing:'-0.01em'}}>Participant profile</h2>
              <div className="grid grid-cols-2 gap-3">
                {[{label:'Full name',value:participant.name},{label:'Email',value:participant.email},{label:'Date of birth',value:participant.dob},{label:'LIABL Pass',value:participant.isReturning?`✦ Active — ${participant.visitCount} visits`:'First visit'},{label:'Health flags',value:participant.healthFlag??'None disclosed'},{label:'Minor',value:participant.isMinor?'Yes — guardian signed':'No'}].map(({label,value})=>(
                  <div key={label} className="bg-surface rounded-xl p-3"><div className="text-xs text-gray-400 mb-1">{label}</div><div className="text-sm font-medium text-ink">{value}</div></div>
                ))}
              </div>
            </div>
          )}
          {section==='audit'&&(
            <div className="bg-white rounded-2xl border border-black/10 overflow-hidden">
              ...
              <div className="divide-y divide-black/5">
                {[{time:'09:04:12.441',event:'waiver.signed',detail:`IP ${participant.ip} · ${participant.hash}`},{time:'09:04:11.028',event:'signature.captured',detail:participant.device},{time:'09:03:58.774',event:'document.viewed',detail:'Participant reviewed all clauses'},{time:'09:03:44.102',event:'waiver.generated',detail:`${participant.activity} · ${participant.clauses.filter(c=>c.adaptive).length} adaptive clauses`},{time:'09:03:40.021',event:'session.started',detail:'Entered flow via operator QR code'}].map((e,i)=>(
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-black/10 p-4 space-y-3">
            ...
            {[{label:'Status',value:'✓ Signed',color:'text-emerald-600'},{label:'Signed at',value:participant.signedAt,color:''},{label:'IP',value:participant.ip,color:''},{label:'Doc ID',value:participant.docId,color:''}].map(({label,value,color})=>(
              <div key={label} className="flex justify-between text-xs"><span className="text-gray-400">{label}</span><span className={`font-mono font-medium ${color}`}>{value}</span></div>
            ))}
          </div>
          <div className="bg-white rounded-2xl border border-black/10 overflow-hidden">
            ...
            <div className="divide-y divide-black/5">
              {notes.map(note=>(
                <div key={note.id} className="px-4 py-3"><div className="flex items-center gap-2 mb-1"><span className="text-xs font-medium text-ink">{note.author}</span><span className="text-xs text-gray-400">{note.time}</span></div><p className="text-xs text-gray-600 leading-relaxed">{note.content}</p></div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-2xl border border-black/10 overflow-hidden">
            ...
            <div className="p-4">
              {!addendumSent?(
                ...
                <div className={`text-xs rounded-xl p-3 ${addendumSigned?'bg-emerald-50 text-emerald-700':'bg-amber-50 text-amber-700'}`}>{addendumSigned?'✓ Addendum signed and appended':'⟳ Awaiting participant signature'}</div>
              )}
            </div>
          </div>
        </div>

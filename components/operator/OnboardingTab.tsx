...
export default function OnboardingTab() {
  ...
  const STEPS = [
    { label:'Create account',       icon:'🏢', done: step > 0 },
    { label:'Configure activity',   icon:'⚙️',  done: step > 1 },
    { label:'Invite participant',   icon:'✉️',  done: step > 2 },
    { label:'Watch it work',        icon:'✅', done: step > 3 },
  ]
        ...
        {step === 4 && (
          <div className="card text-center animate-fade-up">
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              {[
                { icon:'📋', label:'View your roster',    href:'/operator' },
                { icon:'⚙️',  label:'Add more activities', href:'/operator' },
                { icon:'🔗', label:'Connect FareHarbor',  href:'/integrations' },
              ].map(({ icon, label, href }) => (

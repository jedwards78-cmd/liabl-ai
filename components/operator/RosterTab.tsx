...
const CLAUSES=[{title:'Assumption of Risk',body:'Participant acknowledges the inherent risks of Whitewater Kayaking and voluntarily assumes full responsibility for any injuries or damages.',adaptive:false},{title:'Release of Liability',body:'Participant releases the operator from all liability arising from participation in Whitewater Kayaking.',adaptive:false},{title:'Water Hazards',body:'Participant acknowledges Class III–IV rapids, submerged obstacles, and risk of capsize. Confirms they are a confident swimmer.',adaptive:false},{title:'Equipment & Safety Briefing',body:'Participant confirms receipt of a full safety briefing and proper fitting of all required safety equipment.',adaptive:false}]
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[{label:'Signed',value:signed,color:'text-emerald-600'},{label:'Pending',value:pending,color:'text-amber-600'},{label:'Complete',value:`${pct}%`,color:'text-brand'}].map(({label,value,color})=>(
          <div key={label} className="bg-white rounded-xl border border-black/10 p-4"><div className={`text-2xl font-semibold ${color}`} style={{letterSpacing:'-0.02em'}}>{value}</div><div className="text-xs text-gray-400 uppercase tracking-wider mt-1">{label}</div></div>
        ))}
      </div>
      <div className="w-full h-1.5 bg-black/8 rounded-full mb-6 overflow-hidden"><div className="h-full bg-brand rounded-full" style={{width:`${pct}%`}}/></div>
      <div className="bg-white rounded-2xl border border-black/10 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-black/8">
          <span className="text-sm font-medium">Participants</span>
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400 hidden sm:block">Click signed row to view detail</span>
        ...
        {visible.map((w,i)=>{
          ...
          const isRet=RETURNING.has(w.id), isSigned=!!w.signed_at
          return (
            <div key={w.id} onClick={()=>isSigned&&setSelected(w)} className={`flex items-center gap-3 px-4 py-3 border-b border-black/5 last:border-0 transition-colors ${isSigned?'hover:bg-brand/5 cursor-pointer':'opacity-60'}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5"><span className="text-sm font-medium truncate">{name}</span>{isRet&&<span className="text-xs bg-brand/10 text-brand px-1.5 py-0.5 rounded-full shrink-0">✦ Pass</span>}</div>

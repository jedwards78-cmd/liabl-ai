...
const DEMO_CLAUSES=[{title:'Assumption of Risk',body:'Participant acknowledges the inherent risks of Whitewater Kayaking and voluntarily assumes full responsibility for any injuries or damages.',adaptive:false},{title:'Release of Liability',body:'Participant releases the operator from all liability arising from participation in Whitewater Kayaking.',adaptive:false},{title:'Water Hazards',body:'Participant acknowledges exposure to Class III–IV rapids, submerged obstacles, and risk of capsize. Participant confirms they are a confident swimmer.',adaptive:false},{title:'Equipment & Safety Briefing',body:'Participant confirms receipt of a full safety briefing and proper fitting of all required safety equipment.',adaptive:false}]
...
export default function RosterTab() {
  ...
  useEffect(()=>{
    async function load(){
      try{
        ...
        const {data}=await supabase.from('waivers').select('id,signed_at,activity_key,is_minor,participants(full_name,email)').order('created_at',{ascending:true}).limit(50)
        if(data&&data.length>0){
          const rows=data.map((row:Record<string,unknown>)=>({id:row.id as string,signed_at:row.signed_at as string|null,activity_key:row.activity_key as string,is_minor:row.is_minor as boolean,participants:Array.isArray(row.participants)?(row.participants[0] as Participant)??null:row.participants as Participant|null}))
          setRoster(rows)
        }
      }catch{/*demo*/}
    }
  ...
  if(selected){
    ...
    return <WaiverDetail participant={{name,email:selected.participants?.email??'',dob:'1992-05-14',activity:LABELS[selected.activity_key]??selected.activity_key,signedAt:selected.signed_at?new Date(selected.signed_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'—',ip:'98.112.44.21',device:'Chrome 124 / macOS',docId:`doc_${selected.id}a1b2`,hash:'sha256:7f3a...9c21',isMinor:selected.is_minor,isReturning:isRet,visitCount:isRet?7:1,healthFlag:null,clauses:DEMO_CLAUSES}} onBack={()=>setSelected(null)}/>
  }
      ...
      <div className="bg-white rounded-2xl border border-black/10 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-black/8">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400 hidden sm:block">Click a signed row to view detail</span>
            <div className="flex gap-1.5">
              {(['all','signed','pending'] as Filter[]).map(f=>(
                <button key={f} onClick={()=>setFilter(f)} className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${filter===f?'bg-brand text-white':'bg-surface text-gray-500 hover:bg-black/5'}`}>
        ...
        {visible.map((w,i)=>{
          ...
          return (
            <div key={w.id} onClick={()=>isSigned&&setSelected(w)} className={`flex items-center gap-3 px-4 py-3 border-b border-black/5 last:border-0 transition-colors ${isSigned?'hover:bg-brand/5 cursor-pointer':'opacity-60'}`}>
              ...
              {w.is_minor?<span className="status-guardian shrink-0">Guardian signed</span>:w.signed_at?<span className="status-signed shrink-0">Signed →</span>:<span className="status-pending shrink-0">Pending</span>}
            </div>
          )
        })}
      </div>

interface Props {
  ...
  dark?: boolean
  operatorName?: string
  operatorAccent?: string   // hex color for white-label demo
}

const sizes = {
  sm: { box:24, rx:4, bar:{x:4,y:4,w:3,h:12},  shelf:{x:4,y:12,w:9,h:3},   dot:{cx:17,cy:8,r:2.5},  word:18, tag:0,  gap:7  },
  md: { box:32, rx:5, bar:{x:5,y:5,w:4,h:16},   shelf:{x:5,y:16,w:12,h:4},  dot:{cx:23,cy:10,r:3},   word:24, tag:0,  gap:9  },
...
export default function Logo({ size = 'md', dark = false, operatorName, operatorAccent }: Props) {
  ...
  const tagColor  = dark ? 'rgba(255,255,255,0.55)' : 'rgba(13,14,18,0.45)'

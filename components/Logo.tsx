...
export default function Logo({ size='md', dark=false, operatorName, operatorAccent }: Props) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:s.gap }}>
      ...
      <div>
        <div style={{ fontFamily:'Syne, sans-serif', fontWeight:600, fontSize:s.word, letterSpacing:'0.02em', color:wordColor, lineHeight:1 }}>
          LIABL
        </div>
        {size === 'lg' && !operatorName && (
          <div style={{ fontFamily:'DM Sans, sans-serif', fontWeight:500, fontSize:s.tag, letterSpacing:'0.12em', color:tagColor, marginTop:5 }}>
            Think faster. Decide better.
          </div>
        )}
        {operatorName && (
          <div style={{ fontFamily:'DM Sans, sans-serif', fontWeight:500, fontSize:s.tag, letterSpacing:'0.04em', color:tagColor, marginTop:2 }}>
            {operatorName}
          </div>
        )}
      </div>

          ...
          <div className="grid grid-cols-2 gap-2">
            {(['kayak','hike','atv','climb'] as Activity[]).map(a=>(
              <button key={a!} onClick={()=>setActivity(a)}
                className={`text-left p-3 rounded-xl border text-sm transition-all ${activity===a?'border-brand bg-brand/5 text-brand font-medium':'border-black/10 bg-surface text-gray-600 hover:border-brand/30'}`}>
                {({kayak:'🚣 Kayaking',hike:'🥾 Hiking',atv:'🏎️ ATV Tour',climb:'🧗 Climbing'} as Record<string,string>)[a!]}
              </button>
            ))}
          </div>
        </div>

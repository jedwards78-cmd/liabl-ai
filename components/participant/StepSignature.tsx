...
export default function StepSignature({ onSign, onBack, saving }: Props) {
  ...
  return (
    <div className="card">
      ...
      <h2 className="font-serif text-2xl mb-1">Sign to confirm</h2>
      <p className="text-gray-500 text-sm mb-5">Draw your signature or type your full name below.</p>

      <div className="relative mb-1">
        <canvas
          ref={canvasRef}
          className="w-full h-20 rounded-xl border border-black/20 bg-surface cursor-crosshair block"
      ...
      {hasSig && (
        <button onClick={clear} className="text-xs text-gray-400 hover:text-gray-600 underline mb-3">
          Clear
        </button>
      )}
      ...
      <div className="my-4">
        ...
        <input className="form-input" type="text" placeholder="Jordan Rivera"
          value={typedName} onChange={e => setTypedName(e.target.value)} />
      </div>

      <div className="bg-brand/5 border border-brand/20 text-brand rounded-xl p-3 text-xs mb-5 flex gap-2 items-start">
        <span>🔒</span>
        <span>Signing at {time} — IP address recorded for legal validity under the ESIGN Act.</span>
      </div>

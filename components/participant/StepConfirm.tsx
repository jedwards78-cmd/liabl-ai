  ...
  return (
    <>
      <div className="card text-center">
        ...
        <h2 className="font-serif text-2xl mb-2">You&apos;re all set, {firstName}.</h2>
        <p className="text-gray-500 text-sm mb-2">
          Your signed waiver has been recorded and emailed to{' '}
          <span className="text-ink font-medium">{answers.email}</span>.
        </p>

        {/* Email preview button */}
        <button onClick={() => setShowEmail(true)}
          className="text-xs text-brand underline mb-5 hover:opacity-70 transition-opacity">
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {[
            `Signed at ${time}`,
            ACTIVITY_LABELS[answers.activityKey] ?? 'Activity',
            'ESIGN compliant',
            'LIABL Pass ✦',
          ].map(tag => (
      ...
      {showEmail && (
        <EmailPreview answers={answers} onDismiss={() => setShowEmail(false)} />
      )}
    </>
  )
}

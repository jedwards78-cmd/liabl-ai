...
export interface ParticipantAnswers {
  fullName: string; dob: string; email: string
  activityKey: ActivityKey; healthStatus: HealthStatus
  isMinor: boolean; guardianName?: string
}
...
export function generateClauses(answers: ParticipantAnswers): WaiverClause[] {
  ...
  if (answers.healthStatus === 'cardiac') clauses.splice(2,0,{ id:'cardiac', title:'Physician Clearance — Cardiovascular Condition', highlight:true, required:true, body:`${answers.fullName} has disclosed a cardiovascular or respiratory condition. Participant confirms written physician clearance within 30 days. Participation without valid clearance voids this waiver.` })
  if (answers.healthStatus === 'injury')  clauses.splice(2,0,{ id:'injury', title:'Recent Injury Disclosure', highlight:true, required:true, body:`${answers.fullName} has disclosed a recent injury or surgery. Participant confirms physician clearance for physical activity of this intensity.` })
  if (answers.isMinor && answers.guardianName) clauses.push({ id:'minor', title:'Guardian Authorization', highlight:true, required:true, body:`${answers.guardianName} grants permission for minor ${answers.fullName} to participate and agrees to all terms of this waiver on their behalf.` })
  return clauses
}
